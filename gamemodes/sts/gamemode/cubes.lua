Cube = {mob = 1, level = 1, rarity = 1, strength = 1, entity = ""}
function Cube.randomize() -- RARITY NUMBERS NOT ACCURATE. ASK TERGATIVE.
...
function Cube.canUpgrade(points)
    ...
    if points >= cost then return true else return false end
end
...
function Cube.getEntity()
    for i, ent in ipairs(ents.GetAll()) do
        if ent:GetName() == Cube.entity then
            return ent
        end
    end
end
